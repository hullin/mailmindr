...
mailmindr.controls.TimespanPicker = function TimespanPicker(spanCombo, datePicker, timePicker, options) {
    ...
    // _elements will hold all used elements in this control
    this._elements = {
        timespans: spanCombo,
    ...
    // _events will hold all registered event handlers
    this._events = {
        onSelectTimespan: []
    };

    // create all needed timespans
    let timespans = [];
    let preference = mailmindrCore.getPreferenceInt("common.showTimespans");
    this._logger.log('pref -> ' + preference);
    let systemOnly = preference == 0;
    let userDefinedOnly = preference == 1;
    let systemAndUserDefined = preference == 2;

    if (systemOnly || systemAndUserDefined) {
        // load system timespans
    ...
    // remove all items from timespan combo and add our items
...
mailmindr.controls.TimespanPicker.prototype = {
    ...
    setTimespan: function(value, timepicker, datepicker) {
        ...
        // this._logger.log('selecting: ' + value);
        // this._logger.log(this._logger.explode(timespan));
    ...
    triggerOnSelectTimespan: function() {
        ...
        for each(let trigger in this._events.onSelectTimespan) {
            trigger(dateTime);
        }
    },
